...
import { isValidDate, normalizeDueAt } from '../utils/dueDate';
...
export function createTask(params: {
  ...
  if (params.dueAt !== undefined) {
    if (!isValidDate(params.dueAt)) {
      throw new TaskValidationError('截止日期格式无效');
    }
    params = { ...params, dueAt: normalizeDueAt(params.dueAt) };
  }
...
export function updateTask(id: string, payload: UpdateTaskPayload): Task {
  ...
  if (payload.dueAt !== undefined) {
    ...
    merged.dueAt = normalizeDueAt(payload.dueAt);
  }
