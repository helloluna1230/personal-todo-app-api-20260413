...
// PUT /tasks/:id — update a task (including dueAt)
...
// PATCH /tasks/:id/due-date — set or clear the due date for a task
router.patch('/:id/due-date', (req: Request, res: Response) => {
  try {
    const { dueAt } = req.body as { dueAt?: string };
    const task = updateTask(req.params['id'] as string, { dueAt });
    res.json(task);
  } catch (err) {
    if (err instanceof TaskNotFoundError) {
      res.status(404).json({ message: err.message });
